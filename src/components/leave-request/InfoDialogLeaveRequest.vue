<template>
  <v-dialog
    v-model="leaveRequestStore.infoDialog"
    max-width="700"
    scrollable
    persistent
  >
    <v-card rounded="lg">
      ...
      <v-card-text class="p-4">
        <div v-if="leaveRequestStore.isLoadingSelected">
          <v-row>
            <v-col cols="6" v-for="i in 6" :key="i">
              <v-skeleton-loader type="subtitle, text"></v-skeleton-loader>
            </v-col>
          </v-row>
        ...
        <div v-else-if="leaveRequest" class="space-y-3">
          <v-row>
            <!-- NAMA -->
            <v-col cols="6">
...
function close() {
  leaveRequestStore.infoDialog = false;
  leaveRequestStore.leaveRequestSelected = null;
}
</script>
